...
description: |
  Use after scientific resolution, and normally after project or prior-style adaptation, to refine
  academic papers, theses, rebuttals, and funding proposals while preserving scholarly conventions,
  claims, evidence, numbers, results, citations, LaTeX, technical meaning, and disclosure obligations.
  This skill is not a scientific or integration authority and must not be used for AI-detection or
  disclosure evasion.
---
...
The upstream methodology below is the substantive core of this skill. Repository contracts and the
active role take precedence over any default behavior described by a skill:

- Use only after the scientific content and claim boundaries have been resolved. `prior-style-adapter`
  may precede this optional naturalness pass when the project style requires it.
- This skill may detect or flag claim, evidence, terminology, or interpretation mismatches, but it must
  not independently change claim strength, scope, certainty, hedging, limitations, causal meaning,
  performance interpretation, or scientific direction. Route ambiguity to `research_lead`.
- Upstream examples of softening claims or attaching evidence are audit prompts, not authorization;
  apply such a change only when it is already specified in the resolved scientific brief, otherwise
  flag it for `research_lead` adjudication.
- Preserve numbers, metrics, equations, symbols, citation commands and keys, references, labels,
  cross-references, macros, technical identifiers, dataset/model names, formal terminology, and
  required disclosures. Preserve the upstream claim-evidence checks; a material transformation also
  requires a pre/post differential review with `claim-auditor`, and `citation-management` when
  references may have changed.
- Semantic manuscript work is provisional under `paper/draft/`. A `candidate_writer` executes the
  resolved brief and owns provisional prose; it does not integrate into a protected main manuscript.
  `independent_reviewer` remains strictly read-only. Only `human_researcher` authorizes protected
  integration, and a separately assigned `integration_agent` performs it.
- Never use this skill to conceal AI assistance, evade disclosure, simulate human authorship, or make
  unsupported prose appear scientifically stronger.
...
## Output
For additional extended before/after examples, read `examples/before-after.md` when a concrete transformation example would help.
...
For each manuscript writing slice, report the slice and scientific brief used, the preserved
pre-transformation text, the post-transformation candidate, the material wording changes, the technical
elements checked, and unresolved scientific or citation risks. Keep the candidate under `paper/draft/`
until the applicable scientific, claim/evidence, and citation checks are complete. Acceptance by a lead
or reviewer is not authorization to integrate it into the protected manuscript.

## Upstream provenance

The substantive core and example were imported from:

```text
Repository: https://github.com/AIScientists-Dev/academic-humanizer
Upstream version declared by SKILL.md: 0.3.3
Snapshot branch: main
Snapshot HEAD: 94b88b23703bed7df507acae7d6d5876209a0cdf
git describe --tags --always --dirty: 94b88b2
Imported: 2026-08-29
