...
def _get_provider_key(provider: str) -> Optional[str]:
    """Read provider API key from environment."""
...
- Professional, reviewable draft appearance
...
"""
    ...
    def __init__(
        ...
        Args:
            api_key: Provider API key (or use provider-specific env var)
            verbose: Print detailed progress information
            provider: Image provider: openrouter, google, or openai
        ...
        self.api_key = api_key or _get_provider_key(provider)
        
        # If not found in environment, try loading from .env file
        if not self.api_key:
            _load_env_file()
            self.api_key = _get_provider_key(provider)
        
        if not self.api_key:
            key_names = " or ".join(PROVIDER_ENV_KEYS[provider])
            raise ValueError(
                f"{key_names} not found. Please either:\n"
                f"  1. Set {key_names} in your environment\n"
                "  2. Add the provider key to your .env file\n"
        ...
        if provider == "google":
            ...
            # Nano Banana 2 through OpenRouter.
    ...
    def _extract_google_image_from_response(self, response: Dict[str, Any]) -> Optional[bytes]:
        ...
        try:
            ...
            self._log(f"Error extracting Google image: {str(e)}")
            return None

    def _generate_image_google(self, prompt: str) -> Optional[bytes]:
        """Generate an image directly with Gemini API."""
        try:
            import requests
        except ImportError as exc:
            raise RuntimeError("Missing dependency: requests. Install with: pip install requests.") from exc
        ...
        try:
            ...
            raise RuntimeError(f"Google Gemini returned non-JSON response: HTTP {response.status_code}")
        if response.status_code != 200:
            raise RuntimeError(f"Google Gemini error HTTP {response.status_code}: {response_json}")
        return self._extract_google_image_from_response(response_json)

    def _generate_image_openai(self, prompt: str) -> Optional[bytes]:
        """Generate an image directly with OpenAI Images API."""
        try:
            import requests
        except ImportError as exc:
            raise RuntimeError("Missing dependency: requests. Install with: pip install requests.") from exc
        ...
        try:
            ...
            raise RuntimeError(f"OpenAI returned non-JSON response: HTTP {response.status_code}")
        if response.status_code != 200:
            raise RuntimeError(f"OpenAI error HTTP {response.status_code}: {response_json}")
    ...
    def generate_image(self, prompt: str) -> Optional[bytes]:
        ...
        if self.provider == "google":
            try:
                return self._generate_image_google(prompt)
            except Exception as e:
                self._last_error = str(e)
                self._log(f"âœ— Google generation failed: {self._last_error}")
                return None
        ...
        if self.provider == "openai":
            try:
                ...
                self._last_error = str(e)
                self._log(f"âœ— OpenAI generation failed: {self._last_error}")
                return None
    ...
    def review_image(self, image_path: str, original_prompt: str, 
        ...
        if self.provider != "openrouter":
            ...
            note = (
                f"Automated visual review is currently implemented only for the OpenRouter backend. "
                f"Provider `{self.provider}` generated the image, but manual scientific review is required."
            )
            self._log(note)
            return note, threshold, False
...
Environment:
  ...
  OPENAI_API_KEY        OpenAI API key
    ...
    parser.add_argument("--provider", default="openrouter", choices=["openrouter", "google", "openai"],
                       help="Image provider. OpenRouter keeps the full automated critic loop. Direct providers generate the image and require manual review.")
    ...
    # Check for API key
    if args.provider not in PROVIDER_ENV_KEYS:
        print(f"Error: unsupported provider {args.provider}")
        sys.exit(1)

    _load_env_file()
    api_key = args.api_key or _get_provider_key(args.provider)
    if not api_key:
        key_names = " or ".join(PROVIDER_ENV_KEYS[args.provider])
        print(f"Error: {key_names} environment variable not set")
        print("\nSet it with:")
        print(f"  export {PROVIDER_ENV_KEYS[args.provider][0]}='your_api_key'")
