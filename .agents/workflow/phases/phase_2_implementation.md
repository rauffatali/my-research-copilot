...
5. For implementation-only work, use `docs/agent/implementation_notes.md` for reasoning, engineering decisions, debugging notes, changed files, validation, outputs, blockers, and next step. When the work may affect a material research or engineering claim, also record the intended evidence or decision being supported; code existence alone is not claim evidence.
...
7. Promote a queue item to `docs/agent/experiment_plan.md` only when it has a linked hypothesis or direction, decision target, baseline/control, experimental change, target metric or failure mode, minimal viable version, stop condition, expected outcomes, evidence to be produced, verification method, and scope/claim boundary.
...
9. During experiment execution, record actual runs in `docs/agent/run_registry.md`, link each run to its approved experiment or hypothesis, save configs/logs/metrics/checkpoints/predictions/plots under durable `runs/` and `outputs/` paths, and record verification status and the next decision in `docs/agent/experiment_journal.md`.
...
11. Run narrow validation before expensive or full-scale execution: tests, smoke checks, config validation, data-loader checks, metric dry runs, or small synthetic/subset runs. Save the validation command and outcome when it affects an implementation or claim decision.
...
Do not treat a queued experiment as approved, an approved experiment as completed, or a completed run as interpreted evidence or claim support until Phase 3 consolidates it into result evidence and a result-to-claim mapping.
...
- serious executed experiments have a linked queue item or bypass reason, linked experiment plan, run registry entry, saved config, saved output path, intended evidence target, verification record, scope/claim boundary, and recorded next decision;
